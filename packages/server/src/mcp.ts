...
import { AudioMasterSchema, AnimationNodeSchema, AnimationOperationSchema } from "@mcp-video-studio/contracts";
...
export function createMcpServer(runtime: StudioRuntime, gateway: Gateway): McpServer {
  ...
  for (const name of Object.keys(featureSchemas) as FeatureName[]) {
    server.registerTool(name, { description: name.replaceAll("_", " "), inputSchema: featureSchemas[name], annotations: { readOnlyHint: name.startsWith("compare_") || name.startsWith("export_") || name.startsWith("inspect_"), destructiveHint: name==="retime_clip", openWorldHint: false } }, async (input: unknown) => invoke(() => invokeFeature(runtime, name, input)));
  }
  ...
  server.registerTool("set_audio_master",{description:"Process the combined voice/music mix once, after track buses. Nested sequences retain their own master processing.",inputSchema:z.object({...projectRevision,sequenceId:z.string(),master:AudioMasterSchema}),annotations:{destructiveHint:true,openWorldHint:false}},async({projectPath,expectedRevision,...command})=>invoke(()=>runtime.apply(projectPath,expectedRevision,[{type:"audio.master.set",...command}])));
  server.registerTool("duck_audio_under_voice",{description:"Apply sample-aligned music ducking under enabled voice clip regions, with attack/release ramps. This follows timeline regions, not speech detection; reapply after changing voice timing.",inputSchema:z.object({...projectRevision,sequenceId:z.string(),musicTrackId:z.string(),voiceTrackIds:z.array(z.string()).min(1).max(1000),attenuationDb:z.number().min(-60).max(0),attackTick:z.number().int().nonnegative(),releaseTick:z.number().int().nonnegative()}),annotations:{destructiveHint:true,openWorldHint:false}},async({projectPath,expectedRevision,...command})=>invoke(()=>runtime.apply(projectPath,expectedRevision,[{type:"audio.duck",...command}])));
  server.registerTool("set_audio_parameter_range", {
    description:"Set pan or an existing numeric effect parameter over an exact sample interval. Stateful processing retains full history; surrounding program samples are preserved. Hold ranges only, at most 16 distinct setting combinations.",
    inputSchema:z.object({...projectRevision,sequenceId:z.string(),targetType:z.enum(["clip","track"]),targetId:z.string(),parameter:z.enum(["pan","effect"]),effectId:z.string().optional(),parameterKey:z.string().optional(),startTick:z.number().int().nonnegative(),endTick:z.number().int().positive(),value:z.number().finite()}),
    annotations:{destructiveHint:true,openWorldHint:false}
  },async({projectPath,expectedRevision,effectId,parameterKey,...command})=>invoke(()=>runtime.apply(projectPath,expectedRevision,[{type:"audio.parameter.range",...command,...(effectId?{effectId}:{}),...(parameterKey?{parameterKey}:{})}])));
  ...
  server.registerTool("export_project_archive",{description:"Queue a cancellable, atomic checksummed portable project archive. Returns job/operation IDs; use get_job for the durable result.",inputSchema:z.object({...projectRevision,outputPath:z.string().min(1)}),annotations:{destructiveHint:false,openWorldHint:false}},async({projectPath,outputPath,expectedRevision})=>invoke(()=>queueArchive(runtime,"export",{projectPath,outputPath,expectedRevision})));
  ...
  server.registerTool("import_project_archive",{description:"Queue a cancellable, checksum-validated import into a new directory. Existing directories are rejected; get_job returns the completed project.",inputSchema:z.object({filePath:z.string().min(1),destinationPath:z.string().min(1)}),annotations:{destructiveHint:false,openWorldHint:false}},async({filePath,destinationPath})=>invoke(()=>queueArchive(runtime,"import",{filePath,destinationPath})));
  server.registerTool("queue_media_consolidation",{description:"Queue cancellable hash-verified consolidation of linked media. Poll get_job; canonical project changes only after copies verify.",inputSchema:z.object({...projectRevision,mediaIds:z.array(z.string()).max(10000).optional()}),annotations:{destructiveHint:false,openWorldHint:false}},async({mediaIds,...input})=>invoke(()=>runtime.queueConsolidation({...input,...(mediaIds?{mediaIds}:{})})));
  server.registerTool("list_export_history",{description:"List saved export snapshots, input/output hashes and publication outcomes for a project.",inputSchema:z.object({projectPath:z.string()}),annotations:{readOnlyHint:true,openWorldHint:false}},async({projectPath})=>invoke(()=>listExportHistory(runtime.config,projectPath)));
  server.registerTool("get_export_history",{description:"Read a saved export project snapshot, preset, source hashes and exact application/FFmpeg environment.",inputSchema:z.object({exportId:z.string().uuid()}),annotations:{readOnlyHint:true,openWorldHint:false}},async({exportId})=>invoke(()=>getExportHistory(runtime.config,exportId)));
  server.registerTool("reproduce_export",{description:"Queue the saved snapshot without changing the current project. Rejects changed source hashes or application/FFmpeg environment.",inputSchema:z.object({projectPath:z.string(),exportId:z.string().uuid(),outputPath:z.string()}),annotations:{destructiveHint:false,openWorldHint:false}},async(input)=>invoke(()=>queueExport(runtime,{...input,reproduceId:input.exportId,sequenceId:'',presetId:''})));
  server.registerTool("list_archive_operations",{description:"Inspect persisted archive outcomes and interrupted-operation recovery before retrying.",annotations:{readOnlyHint:true,openWorldHint:false}},async()=>invoke(()=>listArchiveJobs(runtime.config)));
  return server;
}
