...
export function createMcpServer(runtime: StudioRuntime, gateway: Gateway): McpServer {
  ...
  for (const name of Object.keys(featureSchemas) as FeatureName[]) {
    server.registerTool(name, { description: name.replaceAll("_", " "), inputSchema: featureSchemas[name], annotations: { readOnlyHint: name.startsWith("compare_") || name.startsWith("export_") || name.startsWith("inspect_"), destructiveHint: false, openWorldHint: false } }, async (input: unknown) => invoke(() => invokeFeature(runtime, name, input)));
  }
