...
// Focus traversal must not create a competing revision before the next action.
// Compare the actual displayed value before submitting a blur edit.
function changedNumber(input:HTMLInputElement,current:number){
 return input.checkValidity()&&Number.isFinite(input.valueAsNumber)&&input.valueAsNumber!==current;
}
...
export function ClipAudioTools({project,clip,sequence,onMutate}:{project:StudioProject;clip:Clip;sequence:Sequence;onMutate(commands:ProjectCommand[]):Promise<void>}){
 ...
 return <><fieldset><legend>Pan and fades</legend><label>Clip pan<input type="number" min="-1" max="1" step=".05" defaultValue={clip.audio.pan} key={clip.id+"pan"+clip.audio.pan} onBlur={event=>{if(changedNumber(event.currentTarget,clip.audio.pan))patch({...clip.audio,pan:event.currentTarget.valueAsNumber});}}/></label>
 {(["fadeInTick","fadeOutTick"] as const).map(key=><label key={key}>{key==="fadeInTick"?"Fade in":"Fade out"} seconds<input type="number" min="0" max={ticksToSeconds(clip.durationTick)} step=".01" defaultValue={ticksToSeconds(clip.audio[key])} onBlur={event=>{if(changedNumber(event.currentTarget,ticksToSeconds(clip.audio[key])))patch({...clip.audio,[key]:secondsToTicks(event.currentTarget.valueAsNumber)});}}/></label>)}
...
export function AudioMixer({project,sequence,onMutate}:{project:StudioProject|undefined;sequence:Sequence|undefined;onMutate(commands:ProjectCommand[]):Promise<void>}){
 ...
 <label>Track gain {track.name}<input type="number" min="-120" max="24" step=".5" defaultValue={track.gainDb} key={track.gainDb} onBlur={event=>{if(changedNumber(event.currentTarget,track.gainDb))void onMutate([{type:"track.update",sequenceId:sequence.id,trackId:track.id,patch:{gainDb:event.currentTarget.valueAsNumber}}]);}}/></label>
 <label>Track pan {track.name}<input type="number" min="-1" max="1" step=".05" defaultValue={track.pan} key={track.pan} onBlur={event=>{if(changedNumber(event.currentTarget,track.pan))void onMutate([{type:"track.update",sequenceId:sequence.id,trackId:track.id,patch:{pan:event.currentTarget.valueAsNumber}}]);}}/></label>
