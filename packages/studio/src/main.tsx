...
function WorkflowTools({project,sequence,projectPath,selectedClip,playhead,onProject,onMutate,onClose,onError}:{project:StudioProject;sequence:Sequence;projectPath:string;selectedClip:Clip|undefined;playhead:number;onProject(project:StudioProject):void;onMutate(commands:ProjectCommand[]):Promise<void>;onClose():void;onError(message:string):void}) {
  const [text,setText]=useState("Your title"),[preset,setPreset]=useState("title"),[captions,setCaptions]=useState(""),[format,setFormat]=useState<"srt"|"vtt">("srt"),[mediaId,setMediaId]=useState(""),[gapStart,setGapStart]=useState(0),[gapEnd,setGapEnd]=useState(1),[busy,setBusy]=useState(false);
  ...
  return <div className="modal-backdrop"><section className="generation-modal" role="dialog" aria-modal="true" aria-label="Editing workflows"><div className="modal-head"><h1>Editing workflows</h1><button aria-label="Close editing workflows" onClick={onClose}>×</button></div><div className="generation-layout">
    ...
    <section><fieldset><legend>Portable project archive</legend><label>Archive output path<input value={archiveOutput} onChange={event=>setArchiveOutput(event.currentTarget.value)}/></label><button disabled={busy} onClick={()=>void exportArchive()}>Export project archive</button><p role="status">{archiveMessage}</p></fieldset><fieldset><legend>Caption interchange</legend><label>Caption format<select value={format} onChange={e=>setFormat(e.currentTarget.value as "srt"|"vtt")}><option value="srt">SRT</option><option value="vtt">WebVTT</option></select></label><label>Caption file<input type="file" accept=".srt,.vtt,text/vtt" onChange={e=>{const file=e.currentTarget.files?.[0];if(file){if(file.size>1_000_000){onError("Caption file exceeds one MB.");return;}setFormat(file.name.endsWith(".vtt")?"vtt":"srt");void file.text().then(setCaptions);}}}/></label><label>Caption source<textarea rows={12} value={captions} onChange={e=>setCaptions(e.currentTarget.value)}/></label><button disabled={!captions||busy} onClick={()=>void feature("import_captions",{format,text:captions,trackId:sequence.tracks.find(t=>t.type==="caption")?.id})}>Import captions</button><button disabled={busy} onClick={()=>void feature("export_captions",{format})}>Download caption sidecar</button></fieldset></section>
  </div></section></div>;
}
