...
function Preview({ project, projectPath, sequence, selectedClip, selectedMedia, playhead, ready, busy, onBuild, onPlayhead }: { project: StudioProject | undefined; projectPath: string; sequence: Sequence | undefined; selectedClip: Clip | undefined; selectedMedia: MediaAsset | undefined; playhead: number; ready: boolean; busy: boolean; onBuild(): Promise<void>; onPlayhead(tick: number): void }) {
  ...
  </div><div className="transport"><button aria-label="Previous frame" title="Previous frame" disabled={!ready} onClick={() => step(-1)}>◀</button><button aria-label="Play or pause" title="Play or pause" disabled={!project || busy} onClick={() => void togglePlayback()}>{busy ? "…" : playing ? "Ⅱ" : "▶"}</button><button aria-label="Next frame" title="Next frame" disabled={!ready} onClick={() => step(1)}>▶|</button><code>{project ? formatTimecode(playhead, project.settings.fps) : "00:00:00:00"}</code><span>{sequence?.name ?? "No sequence"}</span></div><ProgramAudioMeter media={videoRef} sourceKey={previewUrl}/></section>;
}
...
function CaptionInspector({ project, sequence, caption, onMutate }: { project: StudioProject; sequence: Sequence; caption: CaptionCue; onMutate(commands: ProjectCommand[]): Promise<void> }) {
  ...
  return <aside className="panel inspector"><div className="tabs"><button className="active">Caption</button></div><div className="inspector-body caption-inspector">
    ...
    <fieldset><legend>Text</legend><label>Caption text<textarea aria-label="Caption text" key={`${caption.id}-text-${caption.text}`} rows={4} defaultValue={caption.text} onBlur={(event) => event.currentTarget.value.trim() && update({ text: event.currentTarget.value.trim() })} /></label></fieldset>
