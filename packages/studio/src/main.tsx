...
function App() {
  ...
  return <div className={`app-shell display-${display.mode}`}>
    ...
    <main className="workspace">
      ...
      {selectedCaption && project && sequence ? <CaptionInspector project={project} projectPath={projectPath} sequence={sequence} caption={selectedCaption} onMutate={mutate} /> : <Inspector project={project} sequence={sequence} clip={selectedClip} jobs={jobs} projectPath={projectPath} onUpdate={updateSelected} onMutate={mutate} onError={setError} onNavigate={(tick,ids,captionIds)=>{setPlayhead(tick);selectClips(ids);if(captionIds?.[0])setSelectedCaptionId(captionIds[0]);}} />}
    </main>
...
function ProjectChooser({ projects, initialPath, onRefresh, onOpen, onClose }: { projects: Array<{ path: string; name: string; revision: number }>; initialPath: string; onRefresh(): Promise<void>; onOpen(path: string): Promise<void>; onClose(): void }) {
  ...
  const importArchive=async()=>{setBusy(true);try{const result=await api<{projectPath:string}>("/api/archive/import",{method:"POST",body:JSON.stringify({filePath:archivePath,destinationPath:customPath})});await onRefresh();await onOpen(result.projectPath);}catch(error){setArchiveError(error instanceof Error?error.message:String(error));}finally{setBusy(false);}};
...
function MediaBin({ project, projectPath, artifactVersion, onProject, onImportError, onAdd }: { project: StudioProject | undefined; projectPath: string; artifactVersion: number; onProject(project: StudioProject): void; onImportError(error: string): void; onAdd(media: MediaAsset): void }) {
  ...
  return <aside className="panel media-bin"><div className="panel-title"><span>Media</span><small>{project?.media.length ?? 0}</small></div>
    ...
    <div className="asset-list">{project?.media.map((media) => <button key={media.id} className="asset" title={media.kind==="font"?"Choose this font in caption styles":media.name} onDoubleClick={() => {if(["audio","video","image"].includes(media.kind))onAdd(media);}}><span className={`asset-type ${media.kind}`}><img alt="" src={mediaArtifactUrl(projectPath, media.id, media.probe.hasVideo ? "thumbnail" : "waveform", artifactVersion)} loading="lazy" onLoad={(event) => { event.currentTarget.hidden = false; }} onError={(event) => { event.currentTarget.hidden = true; }} /><i>{media.kind === "audio" ? "♫" : media.kind === "image" ? "▧" : "▶"}</i></span><span><strong>{media.name}</strong><small>{media.kind} · {secondsLabel(media.probe.durationTick)}</small></span></button>)}</div>
  </aside>;
}
...
function Preview({ project, projectPath, sequence, selectedClip, selectedMedia, playhead, ready, busy, onBuild, onPlayhead }: { project: StudioProject | undefined; projectPath: string; sequence: Sequence | undefined; selectedClip: Clip | undefined; selectedMedia: MediaAsset | undefined; playhead: number; ready: boolean; busy: boolean; onBuild(): Promise<void>; onPlayhead(tick: number): void }) {
  ...
  </div><div className="transport"><button aria-label="Safe areas" aria-pressed={safeGuides} onClick={()=>setSafeGuides(value=>!value)}>Safe areas</button><button aria-label="Previous frame" title="Previous frame" disabled={!ready} onClick={() => step(-1)}>◀</button><button aria-label="Play or pause" title="Play or pause" disabled={!project || busy} onClick={() => void togglePlayback()}>{busy ? "…" : playing ? "Ⅱ" : "▶"}</button><button aria-label="Next frame" title="Next frame" disabled={!ready} onClick={() => step(1)}>▶|</button><code>{project ? formatTimecode(playhead, project.settings.fps) : "00:00:00:00"}</code><span>{sequence?.name ?? "No sequence"}</span></div><ProgramAudioMeter media={videoRef} sourceKey={previewUrl}/></section>;
}
...
function Inspector({ project, sequence, clip, jobs, projectPath, onUpdate, onMutate, onError,onNavigate }: { project: StudioProject | undefined; sequence: Sequence | undefined; clip: Clip | undefined; jobs: JobRecord[]; projectPath: string; onNavigate(tick:number,clipIds:string[],captionIds?:string[]):void; onUpdate(command: ProjectCommand & { type: "clip.update" }): void; onMutate(commands: ProjectCommand[]): Promise<void>; onError(error: string): void }) {
  ...
  const render = async () => {
    ...
    try { await api("/api/render", { method: "POST", body: JSON.stringify({ projectPath, sequenceId: sequence.id, presetId, outputPath }) }); setTab("jobs"); }
    catch (caught) { onError(caught instanceof Error ? caught.message : String(caught)); }
  };
...
function EditTools({project,sequence,clip,onMutate}:{project:StudioProject;sequence:Sequence;clip:Clip;onMutate(commands:ProjectCommand[]):Promise<void>}) {
  ...
  return <>
    <fieldset><legend>Timeline operations</legend><label>Frames to adjust<input type="number" step="1" value={frames} onChange={e=>setFrames(e.currentTarget.valueAsNumber)} /></label>
      ...
      <p className="hint">Roll and slide require adjacent ungrouped clips with source handles.</p>
    </fieldset>
...
function WorkflowTools({project,sequence,projectPath,selectedClip,playhead,onProject,onMutate,onClose,onError}:{project:StudioProject;sequence:Sequence;projectPath:string;selectedClip:Clip|undefined;playhead:number;onProject(project:StudioProject):void;onMutate(commands:ProjectCommand[]):Promise<void>;onClose():void;onError(message:string):void}) {
  ...
  const [archiveOutput,setArchiveOutput]=useState(projectPath.replace(/\\/g,"/")+"/exports/project.mcpstudio"),[archiveMessage,setArchiveMessage]=useState("");
  const exportArchive=async()=>{setBusy(true);try{const result=await api<{outputPath:string}>("/api/archive/export",{method:"POST",body:JSON.stringify({projectPath,expectedRevision:project.revision,outputPath:archiveOutput})});setArchiveMessage("Archive saved: "+result.outputPath);}catch(error){onError(error instanceof Error?error.message:String(error));}finally{setBusy(false);}};
  ...
  return <div className="modal-backdrop"><section className="generation-modal" role="dialog" aria-modal="true" aria-label="Editing workflows"><div className="modal-head"><h1>Editing workflows</h1><button aria-label="Close editing workflows" onClick={onClose}>×</button></div><div className="generation-layout">
    ...
    <section><fieldset><legend>Portable project archive</legend><label>Archive output path<input value={archiveOutput} onChange={event=>setArchiveOutput(event.currentTarget.value)}/></label><button disabled={busy} onClick={()=>void exportArchive()}>Export project archive</button><p role="status">{archiveMessage}</p></fieldset><fieldset><legend>Caption interchange</legend><label>Caption format<select value={format} onChange={e=>setFormat(e.currentTarget.value as "srt"|"vtt"|"ass")}><option value="srt">SRT</option><option value="vtt">WebVTT</option><option value="ass">ASS</option></select></label><label>Caption file<input type="file" accept=".srt,.vtt,.ass,text/vtt" onChange={e=>{const file=e.currentTarget.files?.[0];if(file){if(file.size>1_000_000){onError("Caption file exceeds one MB.");return;}setFormat(file.name.toLowerCase().endsWith(".ass")?"ass":file.name.toLowerCase().endsWith(".vtt")?"vtt":"srt");void file.text().then(setCaptions);}}}/></label><label>Caption source<textarea rows={12} value={captions} onChange={e=>setCaptions(e.currentTarget.value)}/></label><button disabled={!captions||busy} onClick={()=>void feature("import_captions",{format,text:captions,trackId:sequence.tracks.find(t=>t.type==="caption")?.id})}>Import captions</button><button disabled={busy} onClick={()=>void feature("export_captions",{format})}>Download caption sidecar</button></fieldset></section>
  </div></section></div>;
}
